...
def train(args, train_loader, model, criterion, optimizer, epoch, train_set):
    ...
    for i, (input,input_new, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        im_add,_,label_add=train_set[index]
        add_mark1=label_add
        ...
        if False:
            ...
            image = unnormalize(input_new[0].cpu())
            image = image.data.cpu().numpy()
            image = np.uint8(image * 255).transpose(1,2,0)
            image = cv2.resize(image, (1024, 1024))
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            image2 = unnormalize(input[0].cpu())
            image2 = image2.data.cpu().numpy()
            image2 = np.uint8(image2 * 255).transpose(1,2,0)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print('test_loss %.4f - test_score %.4f - test_Sp %.4f - test_Sen %.4f'
              % (test_loss, test_score, test_sp, test_sen))
