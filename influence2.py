...
from saliency2.fullgrad import FullGrad
from saliency2.simple_fullgrad import SimpleFullGrad
from saliency2.smooth_fullgrad import SmoothFullGrad

from saliency2.gradcam import GradCAM
from saliency2.grad import InputGradient
from saliency2.smoothgrad import SmoothGrad
...
def influence(args, val_loader, model, criterion,type='train'):
    for batch_idx, (input,input_new, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        'fullgrad': FullGrad(model,im_size = (3,args.img_size,args.img_size)),
        'simple_fullgrad': SimpleFullGrad(model,im_size = (3,args.img_size,args.img_size)),
        ...
        for i in range(input.size(0)):
            ...
            image233 = unnormalize(input[i].cpu())
            ...
            cv2.imwrite(filename + '_origin' + '.jpg', image233)
        for s in saliency_methods:
            saliency_map = saliency_methods[s].saliency(input_image,target_class)
            # Save saliency maps
            for i in range(input.size(0)):
                filename = save_path + str( (batch_idx+1) * (i+1))
                image = unnormalize(input[i].cpu())
...
def main():
    ...
    assert 0==1
    '''
    val_loss, val_score, val_sp, val_sen = validate(args, test_loader, model, criterion)
    print(val_loss, val_score, val_sp, val_sen)
    assert 0==1
    influence(args, train_loader, model, criterion,type='train')
    influence(args, val_loader, model, criterion,type='val')
    assert 0==1
